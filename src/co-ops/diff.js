...
clearDirectory(scratchDir + 'co-ops/diffed/');
clearDirectory(scratchDir + 'co-ops/diffed/modified/bystate/');
clearDirectory(scratchDir + 'co-ops/diffed/co-ops_only/bystate/');
clearDirectory(scratchDir + 'co-ops/diffed/osm_only/');

const osm = JSON.parse(readFileSync(scratchDir + 'osm/co-ops/all.json'));
const coOps = JSON.parse(readFileSync(scratchDir + 'co-ops/formatted/all.geojson'));
...
for (let state in updatedByState) {
    writeFileSync(scratchDir + 'co-ops/diffed/modified/bystate/' + state + '.osc', osmChangeXmlForFeatures(updatedByState[state]));
    console.log("  " + state + ": " + updatedByState[state].length);
}
writeFileSync(scratchDir + 'co-ops/diffed/modified/all.osc', osmChangeXmlForFeatures(updated));

console.log('In CO-OPS but not OSM, needs review and upload: ' + coOpsOnlyFeatures.length);

for (let state in coOpsOnlyByState) {
    writeFileSync(scratchDir + 'co-ops/diffed/co-ops_only/bystate/' + state + '.geojson', JSON.stringify(geoJsonForFeatures(coOpsOnlyByState[state]), null, 2));
    console.log("  " + state + ": " + coOpsOnlyByState[state].length);
}
writeFileSync(scratchDir + 'co-ops/diffed/co-ops_only/all.geojson', JSON.stringify(geoJsonForFeatures(coOpsOnlyFeatures), null, 2));

writeFileSync(scratchDir + 'co-ops/diffed/osm_only/all.json', JSON.stringify(osmOnlyFeatures, null, 2));
