...
clearDirectory(scratchDir + 'co-ops/formatted/');

console.log('Formatting NOAA CO-OPS stations…');

const sourceStations = JSON.parse(readFileSync(scratchDir + 'co-ops/source/all.json')).stations;

const ndbcStations = JSON.parse(readFileSync(scratchDir + 'ndbc/all.json')).stations.children.map(item => item.station);
...
console.log(`Writing data to scratchDir + 'co-ops/formatted/all.geojson'`);
writeFileSync(scratchDir + 'co-ops/formatted/all.geojson', JSON.stringify(json, null, 2));
